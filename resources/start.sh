...
GF_DOMAIN=domain1
GF_ASADMIN=/opt/glassfish/bin/asadmin
GF_DOMAINBASEDIR=/opt/glassfish/glassfish/domains
GF_DOMAINDIR=$GF_DOMAINBASEDIR/$GF_DOMAIN
GF_TEMP_PASSFILE=/opt/glassfish/.asadminpass
CASTORE=$GF_DOMAINDIR/config/cacerts.jks
KEYSTORE=$GF_DOMAINDIR/config/keystore.jks
KEYTOOL=keytool

#
# init glassfish domain dir
#
function gfInit {
...
if [ ! -d "$GF_DOMAINDIR" ]; then
  gfInit
fi
$GF_ASADMIN start-domain --watchdog
